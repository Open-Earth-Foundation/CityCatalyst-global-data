...
Status: **proposal / for review**. Design for landing the Chile climate-finance work into `CityCatalyst/global-api`. Companion to `methodology.md` (the scoring logic) and `finance_db/README.md` (the conceptual model and the CSV fixture). This doc covers the *production* shape: tables, the score, the API, and a phased rollout.

Author context: drafted 2026-06-19 from the v1 `finance_db` fixture and the current global-api `modelled` schema conventions.
