...
- `br-emissions-reduction-potential/` — Brazil **emissions-reduction potential evidence layer**. Brings the 205 in-scope M1 shifts together with the Brazil data-source register, TEF parameter requirements and Climate TRACE v5.9.0 solution evidence, then defines the working transparent calculation architecture. Research release 2026-08-26 contains four normalized evidence CSVs and the two M1 source workbooks; the calculation engine, reproducible extraction notebook and fit verdict remain pending.
