"""
Profile actions or policy signals with a `primary_channel` / `secondary_channel` facet —
...
-----
    ...
    python scripts/profile_channel.py --mode signals
...
def signal_user_message(row: dict) -> str:
    return (
        f"Policy signal ID: {row.get('policy_signal_id','')}\n"
        f"Source document: {row.get('source_document_id','')}\n"
        f"Signal type: {row.get('signal_type','')}\n"
        f"Signal label: {row.get('signal_label','')}\n"
        f"Climate relevance: {row.get('climate_relevance','')}\n"
        f"Primary outcome: {row.get('primary_outcome','')}\n"
        f"Primary intervention: {row.get('primary_intervention','')}\n"
...
def needs_profiling(row: dict) -> bool:
    if (row.get("primary_channel") or "").strip():
        return False
    # actions and signals: channel all rows that lack primary_channel (filter later downstream if needed)
    return True
...
async def run(args) -> int:
    ...
    todo = [r for r in rows if needs_profiling(r)]
    ...
    msg_fn = action_user_message if args.mode == "actions" else signal_user_message
    schema = make_schema()
