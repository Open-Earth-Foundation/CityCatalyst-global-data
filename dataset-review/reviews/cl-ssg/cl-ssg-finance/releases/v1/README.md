...
## Recommended ingestion notes

- Preserve original raw headers in landing tables, then map to normalized snake_case aliases in modeled tables.
- Keep `opportunity_id` as business key candidate and `n°_id` as release-local row index only.
- Treat multi-value categorical fields (`eligible_actor_types`, some `gpc_sector` values) as delimited text requiring split/normalization for analytics.

## Action matching artifacts (v1)

- Rubric specification: `action_finance_matching_rubric.md`
- SQL starter query: `action_finance_matching_rubric.sql`

These artifacts provide a sector-first, explainable scoring method to link this finance dataset with action catalogs (for example, the ClimateView transition actions release).

## Normalization artifact (v1)

- SQL normalization query: `funding_database_v1_normalization.sql`

This query keeps raw values while creating normalized fields and arrays, including:

- `eligible_actor_types_array`
- `gpc_sector_array`
- `gpc_subsector_array`

It also standardizes common categorical fields (`status`, `instrument_type`, `source_actor_type`, `geographic_scope`, `sector_scope`) and handles CSV header quirks such as `n°_id` and `Desición `.
