...
- **Country-agnostic vocabulary, mapped to the canonical reference.** Structural fields and their canonical values are universal; a country's own terms are *values*, not the model. Use `gated`, not `BIP-SNI-gated`; `regional`, not `GORE`. Where the project already defines a canonical vocabulary, normalize to it rather than to source labels: e.g. `gpc_sectors` resolves to the five GPC sectors (+ `cross_sector`) from `gpc-framework.md` — fold non-canonical labels onto their canonical token (`buildings → stationary_energy`, `industry → ippu`), and **drop anything off-list** rather than inventing a column to hold it.
- **Model only what has a consumer — no catch-all or speculative columns.** Every modelled column should answer a known API/analysis need. Do **not** add a `source_extras`/`data_quality_flags` catch-all or a `*_detail`/`*_verbatim` column "in case it's useful" — unmodeled source columns are simply dropped. It is cheaper to add a column later (with a real consumer) than to carry, document, and migrate dead ones. Start minimal; let the deliverable pull columns into existence.
