...
The dataset lifecycle is `dataset-discovery` (needs) → `dataset-review` (vet + catalog) → `cc-mage` (pipeline). The `.cursor/skills/` folder holds the skills that drive each stage.
