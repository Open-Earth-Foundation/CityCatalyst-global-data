...
def reasoning_agent_gpc_mapping_stationary_energy_transportation(
    ...
    2. You are already provided with a dataframe 'df' containing various data.
...
# """
#     Your task is to check and verify the output of a previous extraction agent inside the <extracted_data_actval_stationary_energy_transportation> tags.

#     <extracted_data_actval_stationary_energy_transportation>
#     The output of the previous agent is this: {state.get("extracted_data_actval_stationary_energy_transportation")}.
#     </extracted_data_actval_stationary_energy_transportation>
...
#     3. To complete this task:
#         a. Load the entire dataframe 'df'. This means load all the rows and do not use df.head() to only inspect the first few rows.
#         b. Check the reasoning of the previous agent. This can be found within the <reasoning> tags in the output of the previous agent within <extracted_data_actval_stationary_energy_transportation> tags.
#         c. Verify the mapping that the previous agent had done. The mapping is found within the <extracted_data> tags output of the previous agent. Check if the previous agent has missed any values that are in the datafile but that do not have any mapping or if there is mislabeled data.  For this, check the all unique values in that identified column of the datafile.
#         d. If you approve, return 'APPROVED'. If not, return 'FEEDBACK: [Your feedback here]'.
