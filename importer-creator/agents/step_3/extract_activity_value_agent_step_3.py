...
def extract_activity_value_agent_step_3(
    ...
    task = """
Your task is to extract the Global Protocol for Community-Scale Greenhouse Gas Emission Inventories (GPC) 'activity value' from the provided python pandas dataframe based on the instructions below. You will also create a runnable python script.
...
e. Determine the GPC 'activity value' based on the content of the dataframe 'df', the user provided context in <user_context> tags and the additional context provided within <context_activities> tags. Each row in the dataframe 'df' should be assigned a GPC 'activity value' based on the provided context. To do this you need to inspect the dataframe 'df' row by row and assign each row a GPC 'activity value' based on the information provided in this row.
