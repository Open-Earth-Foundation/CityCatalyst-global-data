...
Each single row in the dataframe 'df' contains unique activity data which can be of a different subsector of the 'Stationary Energy' sector or 'Transportation' sector and therefore can have a different gpc reference number.
Each row can only be associated with one GPC reference number. Never map multiple GPC reference numbers to the same activity data.
...
Make sure to carefully inspect every single data row of the dataframe 'df'.
