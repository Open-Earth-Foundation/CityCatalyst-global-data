...
def create_final_output_agent_initial_script(state: AgentState):
    ...
    input_path_markdown_initially = get_run_path(state, "initial_script/steps/1_initially.md")
    input_path_markdown_deleted_columns = get_run_path(state, "initial_script/steps/2_deleted_columns.md")
    input_path_markdown_datatypes = get_run_path(state, "initial_script/steps/3_datatypes.md")

    # Load the last created python script
    input_path_last_script = get_run_path(state, "initial_script/steps/3_datatypes.py")

    # Load the last created csv file
    input_path_last_csv = get_run_path(state, "initial_script/steps/3_datatypes.csv")

    # Define output paths
    output_path_markdown = get_run_path(state, "initial_script/final/final_output.md")
    output_path_script = get_run_path(state, "initial_script/final/final_output.py")
    output_path_csv = get_run_path(state, "initial_script/final/final_output.csv")
