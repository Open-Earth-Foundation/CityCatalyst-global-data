...
def create_final_output_agent_step_2(state: AgentState):
    ...
    input_path_markdown_extracted_datasource_name = get_run_path(state, "step_2/steps/1_datasource_name.md")
    input_path_markdown_extracted_emissions_year = get_run_path(state, "step_2/steps/2_emissions_year.md")
    input_path_markdown_extracted_actor_name = get_run_path(state, "step_2/steps/3_actor_name.md")
    input_path_markdown_extracted_sector = get_run_path(state, "step_2/steps/4_sector.md")
    input_path_markdown_extracted_sub_sector = get_run_path(state, "step_2/steps/5_sub_sector.md")
    input_path_markdown_extracted_scope = get_run_path(state, "step_2/steps/6_scope.md")
    input_path_markdown_extracted_gpc_refno = get_run_path(state, "step_2/steps/7_gpc_refno.md")

    # Load the last created python script
    input_path_last_script = get_run_path(state, "step_2/steps/7_gpc_refno.py")
    # Load the last created csv file
    input_path_last_csv = get_run_path(state, "step_2/steps/7_gpc_refno.csv")

    # Define output paths
    output_path_markdown = get_run_path(state, "step_2/final/final_output.md")
    output_path_script = get_run_path(state, "step_2/final/final_output.py")
    output_path_csv = get_run_path(state, "step_2/final/final_output.csv")
