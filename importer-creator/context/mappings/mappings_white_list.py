...
white_list_mapping = {
    ...
    Operator: Kept as it matches the "operator" key, indicating the entity responsible for selling or distributing fuel, essential for understanding the source of emissions (e.g., "YPF").
    ...
    Province: Kept because it maps to the "region" key, specifying the region where the data was collected (e.g., "Tucumán").
    ...
    Marketing/Distribution Channel: Kept because it aligns with the "user_type" key, classifying the distribution channels or sectors involved in the fuel sale (e.g., "Service stations").
    ...
    REF_AREA: Kept because it correspond to the "locode" key, indicating the UN/LOCODE associated with the data (e.g., "AR-BRC").
    Reference area: Kept because it correspond to the "region" key, indicating the geographical area associated with the data (e.g., "California").
    ...
    Unit of measure: Kept since it matches the "units" key, specifying the measurement units of the observed values (e.g., "Kilowatt-hours (kWh)").
    TIME_PERIOD: Retained because it aligns with the "period" key, representing the specific time frame of the data (e.g., "2020-01").
    Source: Kept as it may contain valuable information about the data's origin, which is useful for validation and traceability.
    ...
    OBS_VALUE: Kept because it aligns with the "volume" key, representing the measured quantity of the energy product consumed or produced (e.g., 21080).
    """,
    "columns": {
        "period": {
            "data_type": "datetime",
