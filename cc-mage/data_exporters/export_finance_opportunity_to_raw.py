"""Write the unioned supply-review frame to raw_data.finance_opportunity_staging (replace)."""
from os import path
...
# Names Mage's Postgres export mangles (e.g. status -> _status), which then breaks the
# modelled SQL. Catch them here with a clear message instead of a cryptic UndefinedColumn.
# Extend this set if Mage mangles a name not listed.
...
def export_data_to_postgres(df: DataFrame, **kwargs) -> None:
    ...
    with Postgres.with_config(ConfigFileLoader(config_path, "default")) as loader:
        loader.export(df, "raw_data", "finance_opportunity_staging", index=False, if_exists="replace")
