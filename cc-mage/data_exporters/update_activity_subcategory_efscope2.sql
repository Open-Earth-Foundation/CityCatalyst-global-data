...
SELECT
    ((MD5(CONCAT_WS('-', activity_name, activity_units, (MD5(CONCAT_WS('-', methodology_name, gpc_reference_number))::TEXT))))::UUID) AS activity_id,
    activity_name,
    activity_units,
    (MD5(CONCAT_WS('-', methodology_name, gpc_reference_number))::UUID) AS method_id,
    NULL AS activity_subcategory_type --There is no data for this column
FROM activity_data
