...
with_release AS (
    -- inherit the source-dataset release_id, funder_level, and publisher fields (funder backfill)
    -- from the catalog (built from index.yaml), keyed on source_dataset
    SELECT src.*, c.release_id::UUID AS release_id,
           NULLIF(TRIM(c.funder_level), '') AS funder_level,
...
INSERT INTO modelled.finance_opportunity (
    ...
    funder_channel, provider, instrument, gpc_sectors, eligible_actor,
    ...
    specificity, source_url, legal_basis_url, notes,
