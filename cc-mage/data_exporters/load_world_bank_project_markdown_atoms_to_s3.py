...
SKIP_RULES = [
    ...
    r"\bprocurement\b",
    r"grievance redress",
    r"world bank grievance",
    r"^references?\.?$",
    r"^bibliography",
]
SKIP_COMPILED = [re.compile(p, re.IGNORECASE) for p in SKIP_RULES]

# FIX 1 — expanded DOC_TYPE_MAP with World Bank legacy formats
DOC_TYPE_MAP = {
    "project appraisal document":                       "project_appraisal",
    "staff appraisal report":                           "project_appraisal",
    "memorandum & recommendation of the president":     "project_appraisal",
    "memorandum and recommendation of the president":   "project_appraisal",
    "president's report":                               "project_appraisal",
    "implementation completion and results report":     "completion_report",
    "implementation completion report":                 "completion_report",
    "implementation status results report":             "implementation_status",
    "implementation status and results report":         "implementation_status",
    "implementation status report":                     "implementation_status",
    "mid-term review":                                  "midterm_review",
    "midterm review":                                   "midterm_review",
    "project information document":                     "project_information",
    "project information and integrated safeguards":    "project_information",
    "environmental and social review summary":          "environmental_social",
    "environmental assessment":                         "environmental_social",
}
...
def _load_prompt(kwargs: dict) -> str:
    prompt_path = kwargs.get("system_prompt_path", "prompts/project-atom-extraction.txt")
...
def _normalize_doc_type(raw_doc_type: str) -> str:
    candidate = (raw_doc_type or "").strip().lower()

    # FIX 2 — exact match first
    if candidate in DOC_TYPE_MAP:
        return DOC_TYPE_MAP[candidate]

    # FIX 2 — substring fallback (handles "Implementation Status Report (ISR)" etc.)
    for key, value in DOC_TYPE_MAP.items():
        if key in candidate:
            return value

    # Last resort: slugify
    return re.sub(r"[^a-z0-9]+", "_", candidate).strip("_") or "unknown"
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                print(f"API call failed (attempt {attempt + 1}/{max_retries + 1}): {exc} — retrying in {wait}s")
                time.sleep(wait)
            else:
                raise
...
def export_data_to_s3(df: DataFrame, **kwargs) -> DataFrame:
    """
    Read markdown documents from S3, extract atoms via OpenAI, upload NDJSON atoms to S3.
    """
    ...
    if df is None or df.empty:
        print("No markdown files available for atom extraction.")
        return DataFrame(columns=["source_project_id", "document_stem", "markdown_s3_uri", "atoms_s3_uri", "status"])
    ...
    for _, row in df.iterrows():
        ...
        raw_doc_type      = str(row.get("doc_type", "unknown")).strip()
        doc_type          = _normalize_doc_type(raw_doc_type)
        markdown_s3_uri   = row.get("markdown_s3_uri")
        # FIX 4 — use the original PDF S3 URI as source_url so evidence anchors
        # point to the source file, not the derived markdown
        pdf_s3_uri        = row.get("pdf_s3_uri") or markdown_s3_uri
        atoms_s3_uri      = None
        status            = "failed"

        if not source_project_id or not document_stem or not markdown_s3_uri:
            failed += 1
            output_rows.append({
        ...
        try:
            markdown_bucket, markdown_key = _parse_s3_uri(markdown_s3_uri)
            ...
            for batch in batches:
                payload = {
                    ...
                    "source_url":  pdf_s3_uri,   # FIX 4 — PDF URI not markdown URI
                ...
                for atom in extracted:
                    ...
                    atom["_project_id"]   = source_project_id
                    atom["_doc_type"]     = doc_type
            ...
            # FIX 1 — .ndjson extension
            atoms_key     = (
                f"{destination_prefix}/{source_project_id}/atoms/"
                f"{document_stem}_atoms.{atoms_file_extension}"
            )
            ...
            converted   += 1
            status        = "converted"
            print(f"Extracted {len(atoms)} atoms for {source_project_id}/{document_stem} -> {atoms_s3_uri}")

        except Exception as exc:
            failed += 1
            print(f"Failed atom extraction for source_project_id={source_project_id} "
                  f"markdown_s3_uri={markdown_s3_uri}: {exc}")
    ...
    print(f"Atom extraction complete: converted={converted}, failed={failed}, total={len(df)}")
