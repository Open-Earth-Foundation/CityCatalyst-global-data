"""Read the catalog (dataset-review/catalog/index.yaml) for the finance_project source datasets.
...
SOURCE_IDS = ["cl-ssg-projects", "gcf-projects", "cl-mma-fpa-awards", "cl-conaf-bn-awards"]
...
def load_data(*args, **kwargs):
    ...
    print(f"finance_project source catalog: {len(df)} datasets from index.yaml")
    return df


@test
def test_output(output, *args) -> None:
    assert len(output) == len(SOURCE_IDS), "expected one row per finance_project source"
