"""C40 mitigation actions: S3 triple (en/es/pt) → EN-base table with ``*_i18n`` JSON columns."""
...
@data_loader
def load_c40_climate_actions_mitigation_en_from_s3(*args, **kwargs) -> DataFrame:
    """
    Read C40 high-impact mitigation CSVs (en, es, pt) and return a single table keyed off EN.

    Structural / numeric fields follow the English CSV. Shared descriptive fields become
    JSON columns: ``action_name_i18n``, ``description_i18n``, ``outcome_summary_i18n``,
    ``intervention_summary_i18n``, each a UTF-8 JSON object with keys ``en``, ``es``, ``pt``.

    Pipeline variables (``pipelines/action_pathways_to_modelled/metadata.yaml``):

    - ``source_bucket`` — S3 bucket
    - ``c40_mitigation_s3_key_prefix`` — key prefix (loads ``{prefix}_en.csv``, etc.)
    """
    config_path = path.join(get_repo_path(), "io_config.yaml")
    bucket = str(kwargs["source_bucket"]).strip()
    key_prefix = str(kwargs["c40_mitigation_s3_key_prefix"]).strip()
