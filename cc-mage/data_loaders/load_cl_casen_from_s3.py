...
def load_from_s3(*args, **kwargs):
    """
    Load Chile CASEN 2022 comuna-level indicators from S3 CSV and join with the
    OCHA COD-AB parquet to resolve locode from comuna code.

    CASEN CSV schema (long form, aligned with cl-ine-censo raw_data):
    ...
    query = f"""
        SELECT
            ...
            c.region_nombre,
            c.attribute_type,
            c.attribute_value::VARCHAR   AS attribute_value,
            c.attribute_units,
            c.attribute_category
        FROM read_csv_auto('s3://{bucket_name}/{casen_key}') AS c
        INNER JOIN (
            SELECT DISTINCT
