"""iCARE mitigation actions: S3 triple (en/es/pt) → EN-base table with ``*_i18n`` JSON columns."""
...
@data_loader
def load_icare_climate_actions_mitigation_from_s3(*args, **kwargs) -> DataFrame:
    """
    Read iCARE mitigation CSVs (en, es, pt) and return a single table keyed off EN.

    Same shape as the C40 mitigation loader: EN structural columns plus
    ...
    - ``icare_mitigation_s3_key_prefix`` — key prefix, e.g.
      ``files/icare/icare_climate_actions/release/2026-04-24/climate_actions_mitigation``
    """
    config_path = path.join(get_repo_path(), "io_config.yaml")
    bucket = str(kwargs["source_bucket"]).strip()
    key_prefix = str(kwargs["icare_mitigation_s3_key_prefix"]).strip()
