DROP TABLE IF EXISTS raw_data.ct_manufactoring_staging;

CREATE TABLE raw_data.ct_manufactoring_staging AS 
WITH data_raw AS (
SELECT 
  iso3_country,
  sector,
...
FROM raw_data.ct_manufactoring
...
FROM data_raw a 
INNER JOIN raw_data.ct_manufactoring_mapping b 
ON a.sector = b.sector 
AND a.subsector = b.subsector
WHERE a.subsector != 'aluminum'
),
locode_data AS (
SELECT 		(MD5(CONCAT_WS('-', methodology_name, gpc_reference_number))::UUID) AS method_id,
			...
			activity_name,
			jsonb_build_object(activity_subcategoryname1,activity_subcategoryvalue1,activity_subcategoryname2, activity_subcategoryvalue2, 'data-source', source_name) as activity_subcategory_type,
			gas_name,
