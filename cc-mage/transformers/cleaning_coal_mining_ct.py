...
def transform(data: DataFrame, *args, **kwargs):
    ...
    # Filtering the df 
    required_columns = ['source_name', 'iso3_country', 'subsector', 'source_type', 'start_time', 'lat', 'lon', 
    ...
    data['gpc_refno'] = "I.7.1"

    # Replace gas names
    data['gas_name'] = data['gas_name'].replace({'ch4': 'CH4', 'co2': 'CO2'})

    data['unit_denominator'] = 't' ##tonnes
    data['activity_units'] = 't' ##tonnes

    type_dic = {
        'Underground': {
            'activity_name': 'type-mining',
            'activity_subcategory_type1': 'fugitive-emissions-coal-type',
            'activity_subcategory_name1': 'type-coal-mining-and-handling-underground-mines'
        }, 
        'Surface': {
            'activity_name': 'type-mining',
            'activity_subcategory_type1': 'fugitive-emissions-coal-type',
            'activity_subcategory_name1': 'type-surface-mines-mining'
        }, 
        'Underground & Surface': {
            'activity_name': 'type-mining',
            'activity_subcategory_type1': 'fugitive-emissions-coal-type',
    ...
    for source_type, attributes in type_dic.items():
        mask = data['source_type'] == source_type
    ...
    data.drop(columns=['capacity', 'capacity_units', 'capacity_factor', 'emissions_factor_units', 'subsector', 'temporal_granularity', 'facility_name', 'source_type'], inplace=True)
