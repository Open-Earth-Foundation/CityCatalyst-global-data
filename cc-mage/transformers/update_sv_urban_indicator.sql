WITH raw_sv_index AS (
    SELECT 
        cd_mun municipailty_id,
        b.locode,
        -- TRIM(SPLIT_PART(b.municipality, '(', 1)) AS city_name, 
        -- b._state AS region_code, 
        'social vulnerability urban infrastructure index' as indicator_name,
        replace(social_vulnerability_index___urban_infraestructure, ',', '.')::numeric as indicator_score,
        'Index' as indicator_units,
        2024 as indicator_year, 
        'current' as scenario_name,
        'IPEA' as datasource
    FROM raw_data.ccra_icare_sv_urban a 
...
DROP TABLE raw_data.ccra_icare_sv_urban;
