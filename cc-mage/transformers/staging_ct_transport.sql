DROP TABLE IF EXISTS raw_data.ct_transportation_staging;

CREATE TABLE raw_data.ct_transportation_staging AS 
WITH data_raw AS (
SELECT 
  iso3_country,
  sector,
...
FROM raw_data.ct_transportation
...
FROM data_raw a 
LEFT JOIN raw_data.ct_transportation_mapping b 
...
SELECT 		(MD5(CONCAT_WS('-', methodology_name, gpc_reference_number))::UUID) AS method_id,
			...
			'Transportation' as dataset_name,
